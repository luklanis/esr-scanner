...
public final class OcrInitAsyncTask extends AsyncTask<String, String, Boolean> {
     ...
     */
    public OcrInitAsyncTask(IBase base, TessBaseAPI baseApi, ProgressDialog dialog,
     ...
     * of the OCR engine.
     *
     * @param params [0] Pathname for the directory for storing language data files
     *               to the SD card
     */
    protected Boolean doInBackground(String... params) {
        ...
        if (!destinationFile.exists()) {
            Log.d(TAG,
                    "Language data for " + languageCode + " not found in " + tessdataDir.toString());

            // Check assets for language data to install. If not present,
            // download from Internet
            try {
                Log.d(TAG,
                        "Checking for language data (" + destinationFilenameBase + ".zip) in application assets...");
            ...
            Log.d(TAG,
                    "Language data for " + languageCode + " already installed in " + tessdataDir.toString());
            installSuccess = true;
        }
     ...
     */
    private boolean installZipFromAssets(String sourceFilename, File destinationDir)
            throws IOException, FileNotFoundException {
        // Attempt to open the zip archive
        publishProgress("Uncompressing data for " + languageName + "...", "0");
        ZipInputStream inputStream = new ZipInputStream(base.getContext()
                .getAssets()
                .open("tessdata/" + sourceFilename));
    ...
    protected void onPostExecute(Boolean result) {
        ...
        if (result) {
            base.setBaseApi(baseApi);
            // Restart recognition
            base.resumeOcrEngine();
