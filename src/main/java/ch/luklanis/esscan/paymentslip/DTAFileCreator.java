...
public class DTAFileCreator {
	...
	public CharSequence buildDTA(List<HistoryItem> historyItems) {
		...
		for (int i = 0; i < filteredHistoryItem.size(); i++) {
			...
			if (filteredHistoryItem.get(i).getResult().getType()
				...
				reason = reasonInOneLine != null ? reasonInOneLine
						.split(NEWLINE_PATTERN) : new String[0];
			}

			String addressLine = historyItem.getAddress();
			String[] address = addressLine != null ? addressLine
					.split(NEWLINE_PATTERN) : new String[0];

			CharSequence paddedSequenz = padded(String.valueOf(i + 1), '0', 5,
					false);
			...
			dtaText.append(spacePaddedEnd(ownAddressTemp, 40)).append(
					spacePaddedEnd("", 46)); // Reserve
			...
			if (psResult.getType().equals(EsrResult.PS_TYPE_NAME)) {
				...
				if (account.length() > 5) {
					...
					dtaText.append(spacePaddedEnd(referenzNumber, 27)); // Refernz
																		...
																		// account)
					Log.w(TAG,
							"account only 5 digits long -> this will not work!");
				}
		...
		String totalAmountTemp = totalAmountSplit[0] + ","
				+ padded(totalAmountSplit[1], '0', 3, true);

		// Total Record
		dtaText.append(spacePaddedEnd(totalAmountTemp, 16)).append(
				spacePaddedEnd("", 59)); // Reserve
	...
	public static int validateAddress(String address, PsResult psResult) {
		return validateAddress(
				...
				psResult.getType().equals(EsrResult.PS_TYPE_NAME) ? R.string.msg_address_line_to_long_20
						: R.string.msg_address_line_to_long_24);
	}

	public static int validateAddress(String address) {
		return validateAddress(address, 20,
				R.string.msg_address_line_to_long_20);
	}

	public static int validateAddress(String address, int maxCharsPerLine,
			int errorToReturn) {
		if (address.length() > 0) {
	...
	public static int validateIBAN(String iban) {
		...
		int subIban = Integer.parseInt(ibanNumber.substring(lastEnd,
				subIbanLength));
			...
			while (lastEnd < ibanNumber.length()) {
				if ((lastEnd + subIbanLengthWithModulo) < ibanNumber.length()) {
					...
					subIban = Integer.parseInt(String.format("%s%s",
							lastModulo, ibanNumber.substring(lastEnd, newEnd)));
					lastEnd = newEnd;
				} else {
					subIban = Integer.parseInt(String.format("%s%s",
							lastModulo, ibanNumber.substring(lastEnd)));
		...
		if (historyItems != null) {
			...
			for (int i = 0; i < items.size(); i++) {
				...
				if (nullToEmpty(item.getAmount()).equals("") || 
						Float.parseFloat(item.getAmount()) == 0.00) {
					return res.getString(R.string.msg_amount_is_empty, item
							.getResult().getAccount());
				}
			}
		}
	...
	private String getExecutionDateFormated() {
		...
		SharedPreferences prefs = PreferenceManager
				.getDefaultSharedPreferences(context);
		String day = prefs.getString(PreferencesActivity.KEY_EXECUTION_DAY,
				"26");
	...
	private static CharSequence padded(String text, char pad, int length,
			boolean padEnd) {
