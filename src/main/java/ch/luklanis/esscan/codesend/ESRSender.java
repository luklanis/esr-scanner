...
public class ESRSender extends Service implements IEsrSender {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        prefs = PreferenceManager.getDefaultSharedPreferences(ESRSender.this);
            ...
            try {
                for (Enumeration<NetworkInterface> en = NetworkInterface.getNetworkInterfaces(); en.hasMoreElements(); ) {
                    ...
                    for (Enumeration<InetAddress> enumIpAddr = intf.getInetAddresses(); enumIpAddr.hasMoreElements(); ) {
                        ...
                        if (!inetAddress.isLoopbackAddress() && inetAddress.getAddress().length == 4 && inetAddress
                                .isSiteLocalAddress() && (inetAddress.getAddress()[0] != 10 || intf.getName()
                                .contains("wlan"))) {
    ...
    public void sendToListener(final String dataToSend, final int position) {
        ...
        if (mDataSentHandler != null) {
            Message message = Message.obtain(mDataSentHandler,
                    (sent ? R.id.es_send_succeeded : R.id.es_send_failed));
